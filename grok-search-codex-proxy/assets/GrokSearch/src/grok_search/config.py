...
class Config:
    ...
    def grok_model(self) -> str:
        forced = os.getenv("GROK_FORCE_MODEL")
        if forced:
            forced = forced.strip()
            if forced:
                self._cached_model = forced
                return forced
    ...
    @property
    def force_model(self) -> str:
        """若设置 GROK_FORCE_MODEL，则永远使用该模型（跨会话稳定）。"""
        return (os.getenv("GROK_FORCE_MODEL") or "").strip()

    @property
    def web_search_mode(self) -> str:
        """
        控制 web_search/web_fetch 的执行模式。

        - grok-only（默认）：仅允许 GROK_MODEL 为 grok-*，避免在 gpt/claude 等模型下产生“生成式假检索”。
        - any：允许任意模型（不推荐，仅用于调试/生成式草稿）。
        - disabled：禁用 web_search/web_fetch（仅保留 get_config_info/switch_model 等工具）。
        """
        return os.getenv("GROK_WEB_SEARCH_MODE", "grok-only").strip().lower()
    ...
    def get_config_info(self) -> dict:
        ...
        return {
            ...
            "GROK_FORCE_MODEL": self.force_model or "",
            "GROK_WEB_SEARCH_MODE": self.web_search_mode,
