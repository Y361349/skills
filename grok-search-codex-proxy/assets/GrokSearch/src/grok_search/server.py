...
def _redact_secrets(text: str) -> str:
    """对异常/日志字符串做最小必要脱敏，避免意外回显密钥。"""
    if not text:
        return text
    out = str(text)
    out = re.sub(r"\bsk-[A-Za-z0-9_-]{10,}\b", "sk-*****", out)
    out = re.sub(r"(Authorization:\s*Bearer\s+)([^\s\r\n]+)", r"\1*****", out, flags=re.IGNORECASE)
    out = re.sub(r"(GROK_API_KEY\s*=\s*)([^\s\r\n]+)", r"\1*****", out, flags=re.IGNORECASE)
    return out
...
async def web_search(query: str, platform: str = "", min_results: int = 3, max_results: int = 10, ctx: Context = None) -> str:
    ...
    if mode == "disabled":
        return "能力限制: GROK_WEB_SEARCH_MODE=disabled，已禁用 web_search。"
    if mode not in {"grok-only", "any"}:
        return f"配置错误: GROK_WEB_SEARCH_MODE={mode} 不合法（仅支持 grok-only / any / disabled）。"
    if mode == "grok-only" and not model_lower.startswith("grok-"):
        return (
            f"能力限制: 当前 GROK_MODEL={model} 不属于 grok-*。\n"
            "说明: 本项目的 web_search 依赖“模型内置的联网/搜索能力”。在 gpt/claude 等纯聊天模型下，"
            "往往只能生成看似搜索的内容，但缺少可追溯检索证据。\n"
            "处理建议: 切换到可用的 grok 模型（并确保该模型具备搜索/深度检索能力）；"
            "或将 GROK_WEB_SEARCH_MODE=any（不推荐，仅用于生成式草稿）。"
        )
    ...
    try:
        ...
        error_msg = _redact_secrets(f"{type(e).__name__}: {e}")
        if ctx:
            await ctx.report_progress(error_msg)
...
async def web_fetch(url: str, ctx: Context = None) -> str:
    ...
    model_lower = model.lower()
    if mode == "disabled":
        return "能力限制: GROK_WEB_SEARCH_MODE=disabled，已禁用 web_fetch。"
    if mode not in {"grok-only", "any"}:
        return f"配置错误: GROK_WEB_SEARCH_MODE={mode} 不合法（仅支持 grok-only / any / disabled）。"
    if mode == "grok-only" and not model_lower.startswith("grok-"):
        return (
            f"能力限制: 当前 GROK_MODEL={model} 不属于 grok-*。\n"
            "说明: 本项目的 web_fetch 依赖“模型内置的联网/抓取能力”。在 gpt/claude 等纯聊天模型下，"
            "可能返回编造的网页内容，无法保证与真实网页一致。\n"
            "处理建议: 切换到可用的 grok 模型（并确保该模型具备抓取/深度检索能力）；"
            "或将 GROK_WEB_SEARCH_MODE=any（不推荐，仅用于生成式草稿）。"
        )
...
@mcp.tool(
    ...
    str
        ...
        - `api_key`: The API key (masked for security, always "***", no characters revealed)
        ...
        - `model_access_test`: Result of testing /chat/completions for current `GROK_MODEL`
          - `status`: Call status
          - `model`: The model ID being tested
          - `message`: Status message (truncated)
          - `response_time_ms`: API response time in milliseconds
...
async def get_config_info() -> str:
    ...
    # 添加当前 GROK_MODEL 的可用性测试（/chat/completions）
    model_test = {
        "status": "未测试",
        "model": "",
        "message": "",
    ...
    # 独立测试：当前 GROK_MODEL 是否能成功调用 /chat/completions
    try:
        api_url = config.grok_api_url
        api_key = config.grok_api_key
        model = config.grok_model
        ...
        model_test["response_time_ms"] = round(response_time, 2)

        if response.status_code == 200:
            model_test["status"] = "✅ 可用"
            model_test["message"] = "chat/completions 调用成功"
        else:
            model_test["status"] = "❌ 不可用"
            model_test["message"] = _redact_secrets(f"HTTP {response.status_code}: {response.text[:120]}")

    except httpx.TimeoutException:
        model_test["status"] = "⏱️ 超时"
        model_test["message"] = "chat/completions 请求超时（10秒）"
    except httpx.RequestError as e:
        model_test["status"] = "❌ 失败"
        model_test["message"] = _redact_secrets(f"chat/completions 网络错误: {str(e)}")
    except ValueError as e:
        model_test["status"] = "⚠️ 配置错误"
        model_test["message"] = str(e)
    except Exception as e:
        model_test["status"] = "❌ 异常"
        model_test["message"] = _redact_secrets(f"{type(e).__name__}: {e}")

    config_info["connection_test"] = test_result
    config_info["model_access_test"] = model_test
