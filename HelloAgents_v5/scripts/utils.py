...
class ExecutionReport:
    ...
    用法:
        ...
        # 遇到错误
        report.mark_failed("加载模板 plan-why-template.md", ["创建 why.md", "创建 how.md", "创建 task.md"])
...
def get_history_path(base_path: Optional[str] = None) -> Path:
    """获取 history/ 目录路径"""
    return get_workspace_path(base_path) / "history"
        ...
        if item.is_dir():
            ...
            if parsed:
                ...
                pkg_info = {
                    ...
                    'task_count': count_tasks(item / "task.md")
                }
...
def get_package_summary(package_path: Path) -> str:
    ...
    Returns:
        功能摘要
    """
    why_file = package_path / "why.md"
    if not why_file.exists():
        return "(无描述)"

    content = why_file.read_text(encoding='utf-8')
    # 尝试提取第一个非标题非空行
    lines = content.split('\n')
    for line in lines:
        line = line.strip()
...
def load_template(template_path: str, required: bool = True) -> Optional[str]:
    ...
    Args:
        template_path: 相对于模板目录的路径，如 "plan-why-template.md"
...
def extract_required_sections(template: str) -> List[str]:
    ...
    for section in sections:
        ...
        if not core:
            continue
        # 跳过占位符章节（如 "[核心功能模块名称]"），避免对已定稿方案产生误导性 warnings
...
class TemplateLoader:
    ...
    用法:
        ...
        # 加载模板
        why_template = loader.load("plan-why-template.md")

        # 填充占位符
        content = loader.fill("plan-why-template.md", {
            "[功能名称]": "login"
        })

        # 获取必需章节
        sections = loader.get_required_sections("plan-why-template.md")
    """
